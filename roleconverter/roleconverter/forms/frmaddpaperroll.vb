Public Class frmaddpaperroll
    ...
    Public Sub LoadClass()
        ...
        For Each dr As DataRow In ds.Tables(0).Rows
            ...
            Dim mySqlx As String = "select TBL_ADD_PAPERROLL.QUANTITY AS TOTALQ from TBL_PAPERROLL  INNER JOIN TBL_ADD_PAPERROLL ON TBL_ADD_PAPERROLL.PAPERROLL_ID= TBL_PAPERROLL.PAPERROLE_ID   WHERE TBL_ADD_PAPERROLL.PAPERROLL_ID='" & dr("PAPERROLE_ID") & "'"

            Dim ds1 As DataSet = LoadSQL(mySqlx)

            '   lvList.Items.Clear()
            For Each dr1 As DataRow In ds1.Tables(0).Rows
                TOTALQROLL = Val(dr1("TOTALQ")).ToString
                If TOTALQROLL = Nothing Then
                    lv.SubItems.Add(0)
                Else
    ...
    Private Sub lvList_DOUBLECLICK(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles lvList.DoubleClick
        ...
        If Val(lvList.SelectedItems(0).SubItems(6).Text) > 0 Then

            txtremaining.Text = Val(lvList.SelectedItems(0).SubItems(6).Text).ToString
        End If
    End Sub
